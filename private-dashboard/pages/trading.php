...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save') {
        ...
        if ($id > 0) {
            $db->prepare("UPDATE trading_trades SET datum=?,symbol=?,richtung=?,einstieg=?,ausstieg=?,lots=?,gewinn=?,status=?,strategie=?,notiz=? WHERE id=?")->execute([$datum,$symbol,$richt,$ein,$aus,$lots,$gewinn,$status,$strat,$notiz,$id]);
        } else {
            $db->prepare("INSERT INTO trading_trades (datum,symbol,richtung,einstieg,ausstieg,lots,gewinn,status,strategie,notiz) VALUES (?,?,?,?,?,?,?,?,?,?)")->execute([$datum,$symbol,$richt,$ein,$aus,$lots,$gewinn,$status,$strat,$notiz]);
        }
        header("Location: ?page=trading&msg=saved"); exit;
    }

    if ($act === 'delete') {
        $db->prepare("DELETE FROM trading_trades WHERE id=?")->execute([(int)$_POST['id']]);
        header("Location: ?page=trading&msg=saved"); exit;
    }
}
if (defined('HANDLE_POST_ONLY')) return;

$trades       = $db->query("SELECT * FROM trading_trades ORDER BY datum DESC")->fetchAll();
    ...
    <div class="table-wrap"><table class="data-table">
        ...
        <tr>
            ...
            <td class="col-actions">
                <form method="POST" action="?page=trading" class="form-inline">
                    <?= csrf_field() ?>
...
<div class="card mt-4">
    <div class="card-head"><h2 class="card-title">Alle Trades</h2></div>
    <div class="table-wrap"><table class="data-table">
