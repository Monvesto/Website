<?php
// ════════════════════════════════════════════════
// ziele.php – Ziele mit Bulk-Edit + Person-Filter
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save') {
        ...
        $per  = $_POST['person_ziel'] ?? $def_person;
        $id   = (int)($_POST['edit_id'] ?? 0);
        if ($id > 0) {
            $db->prepare("UPDATE ziele SET ziel=?,kategorie=?,startwert=?,zielwert=?,aktueller_wert=?,zieltermin=?,kommentar=?,person=? WHERE id=? AND user_id=?")->execute([$ziel,$kat,$sw,$zw,$aw,$zt,$kom,$per,$id,$uid]);
        } else {
            $db->prepare("INSERT INTO ziele (user_id,ziel,kategorie,startwert,zielwert,aktueller_wert,zieltermin,kommentar,person) VALUES (?,?,?,?,?,?,?,?,?)")->execute([$uid,$ziel,$kat,$sw,$zw,$aw,$zt,$kom,$per]);
        }
    ...
    if ($act === 'bulk_save') {
        foreach ($_POST['ids'] ?? [] as $id) {
            ...
            $ziel = trim($row['ziel'] ?? ''); if ($ziel === '') continue;
            $db->prepare("UPDATE ziele SET ziel=?,kategorie=?,startwert=?,zielwert=?,aktueller_wert=?,zieltermin=?,kommentar=?,person=? WHERE id=? AND user_id=?")
               ->execute([trim($row['ziel']??''),trim($row['kategorie']??''),parse_betrag($row['startwert']??'0'),parse_betrag($row['zielwert']??'0'),parse_betrag($row['aktueller_wert']??'0'),($row['zieltermin']??'')?:null,trim($row['kommentar']??''),$row['person']??$def_person,$id,$uid]);
        }
        header("Location: ?page=ziele&person=$pf&msg=saved"); exit;
    }

    if ($act === 'delete') {
        $db->prepare("DELETE FROM ziele WHERE id=? AND user_id=?")->execute([(int)$_POST['id'], $uid]);
        header("Location: ?page=ziele&person=$pf&msg=saved"); exit;
    }

    if ($act === 'update_wert') {
        $db->prepare("UPDATE ziele SET aktueller_wert=? WHERE id=? AND user_id=?")->execute([parse_betrag($_POST['aktueller_wert']??'0'),(int)$_POST['id'],$uid]);
        header("Location: ?page=ziele&person=$pf&msg=saved"); exit;
    }
}
...
if ($is_all) {
    $s = $db->prepare("SELECT * FROM ziele WHERE user_id=? ORDER BY position, zieltermin"); $s->execute([$uid]);
} else {
    $s = $db->prepare("SELECT * FROM ziele WHERE user_id=? AND (person=? OR person='Beide') ORDER BY position, zieltermin"); $s->execute([$uid,$person]);
}
$ziele = $s->fetchAll();
...
function sel_z(array $opts, string $cur): string {
    $out = ''; foreach ($opts as $o) $out .= '<option value="'.he_z($o).'"'.($o===$cur?' selected':'').'>'.he_z($o).'</option>'; return $out;
}
function kat_sel_z(array $opts, string $cur): string {
    $out = '<option value="">– wählen –</option>'; foreach ($opts as $o) $out .= '<option value="'.he_z($o).'"'.($o===$cur?' selected':'').'>'.he_z($o).'</option>'; return $out;
}
?>
...
<form id="frm-zl-bulk" method="POST" action="?page=ziele">
    <?= csrf_field() ?><input type="hidden" name="act" value="bulk_save"><input type="hidden" name="person_filter" value="<?= he_z($person) ?>">
    <?php foreach ($ziele as $z): ?><input type="hidden" name="ids[]" value="<?= $z['id'] ?>"><?php endforeach; ?>
</form>

<div class="card mt-4" id="card-ziele">
    <div class="card-head">
        <div class="card-head-left">
            <h2 class="card-title">🎯 Ziele<?= !$is_all?' – '.$person:'' ?></h2>
    ...
    <div class="table-wrap"><table class="data-table">
        <thead><tr>
            <th>Ziel</th><th>Kategorie</th>
            <?php if($is_all): ?><th>Person</th><?php endif; ?>
            <th>Fortschritt</th><th class="col-right">Aktuell</th><th class="col-right">Zielwert</th><th>Termin</th><th></th>
        </tr></thead>
        <tbody>
        <?php foreach ($ziele as $z): $zid=$z['id'];
            $range=abs((float)$z['zielwert']-(float)$z['startwert']); $curr=abs((float)$z['aktueller_wert']-(float)$z['startwert']);
            $prog=$range>0?min(1,$curr/$range):0; $pcls=progress_cls($prog); $pct_r=number_format($prog*100,1,'.',''  );
        ?>
        <tr>
            <td>
                ...
                <?php if ($z['kommentar']): ?><div class="ft-bulk" style="font-size:11px;color:var(--text-muted);margin-top:2px"><?= he_z($z['kommentar']) ?></div><?php endif; ?>
            ...
            <td>
                ...
                <select class="inline-input fi-bulk" form="frm-zl-bulk" name="rows[<?= $zid ?>][person]"><?= sel_z($person_options,$z['person']??$def_person) ?></select>
            </td>
            <?php else: ?>
            <input type="hidden" form="frm-zl-bulk" name="rows[<?= $zid ?>][person]" value="<?= he_z($z['person']??$def_person) ?>">
            <?php endif; ?>
            <td><div class="progress-wrap"><div class="progress-track"><div class="progress-fill <?= $pcls ?>" data-width="<?= $pct_r ?>"></div></div><span class="progress-label <?= $pcls ?>"><?= $pct_r ?>%</span></div></td>
            <td class="col-right">
                <form method="POST" action="?page=ziele" class="ft-bulk" style="display:flex;align-items:center;gap:4px;justify-content:flex-end">
                    <?= csrf_field() ?><input type="hidden" name="act" value="update_wert"><input type="hidden" name="id" value="<?= $zid ?>"><input type="hidden" name="person_filter" value="<?= he_z($person) ?>">
            ...
            <td><span class="ft-bulk"><?= $z['zieltermin']?date('d.m.Y',strtotime($z['zieltermin'])):'–' ?></span><input class="inline-input fi-bulk" type="date" form="frm-zl-bulk" name="rows[<?= $zid ?>][zieltermin]" value="<?= he_z($z['zieltermin']??'') ?>"></td>
            <td class="col-actions">
                <form id="frm-zl-del-<?= $zid ?>" method="POST" action="?page=ziele" hidden>
                    <?= csrf_field() ?><input type="hidden" name="act" value="delete"><input type="hidden" name="id" value="<?= $zid ?>"><input type="hidden" name="person_filter" value="<?= he_z($person) ?>">
                </form>
                <button type="submit" form="frm-zl-del-<?= $zid ?>" class="btn btn-danger btn-xs fi-bulk btn-delete-confirm">✕</button>
            </td>
        </tr>
        <?php endforeach; ?>
        <?php if (empty($ziele)): ?><tr><td colspan="8" class="empty-state">Noch keine Ziele angelegt.</td></tr><?php endif; ?>
        </tbody>
    </table></div>
</div>
...
<div class="card mt-4">
    ...
    <form id="frm-zl-new" method="POST" action="?page=ziele">
        <?= csrf_field() ?><input type="hidden" name="act" value="save"><input type="hidden" name="edit_id" value="0"><input type="hidden" name="person_filter" value="<?= he_z($person) ?>">
        <?php if(!$is_all): ?><input type="hidden" name="person_ziel" value="<?= he_z($def_person) ?>"><?php endif; ?>
    </form>
    <div class="table-wrap"><table class="data-table">
        <thead><tr>
            <th>Ziel</th><th>Kategorie</th><?php if($is_all): ?><th>Person</th><?php endif; ?>
            <th>Aktuell</th><th>Zielwert</th><th>Startwert</th><th>Termin</th><th></th>
        </tr></thead>
        <tbody><tr>
            <td><input class="inline-input new-input" form="frm-zl-new" name="ziel" placeholder="Zielname" required><input class="inline-input new-input" form="frm-zl-new" name="kommentar" placeholder="Kommentar" style="margin-top:3px"></td>
            <td><select class="inline-input new-input" form="frm-zl-new" name="kategorie"><?= kat_sel_z($kategorien,'') ?></select></td>
            <?php if($is_all): ?><td><select class="inline-input new-input" form="frm-zl-new" name="person_ziel"><?= sel_z($person_options,$def_person) ?></select></td><?php endif; ?>
            ...
            <td class="col-actions"><button type="button" class="btn btn-primary btn-xs" id="btn-new-zl">+ Hinzufügen</button></td>
        </tr></tbody>
    </table></div>
</div>